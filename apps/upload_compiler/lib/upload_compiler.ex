defmodule Spire.UploadCompiler do
  ...
  alias Spire.UploadCompiler.StatsCalculations, as: Calculator

  def start_link(_opts) do
    Broadway.start_link(__MODULE__,
      name: __MODULE__,
      producer: [
        module: {BroadwaySQS.Producer, queue_url: Application.get_env(:upload_compiler, :sqs_queue_url)}
      ],
      processors: [
        default: []
      ]
  ...
  def handle_message(_, %Message{data: data} = message, _) do
    ...
    %HTTPoison.Response{body: body, status_code: 200} = HTTPoison.get!("https://logs.tf/json/#{upload["logfile"]}")
    ...
    stats_all = Enum.map(players, fn player ->
      Stats.get_or_create_stats_all_for_player!(player, real)
    end)

    stats_individual = Enum.map(players, fn player ->
    ...
    updated_stats_all = Enum.map(stats_all, fn stat ->
      Calculator.calculate_stats_all(stat,  log_json)
    end)

    updated_stats_individual = Enum.map(stats_individual, fn stat ->
      Calculator.calculate_general_stats_individual(stat,  log_json)
    end)
  ...
  def get_player_stubs(log_json) do
    ...
    |> Enum.map(fn key ->
      ...
      {:ok, %{"personaname" => alias, "avatarfull" => avatar}} = SteamUtils.get_steam_player(steamid64)
  ...
  def persist_stats(stats, upload) do
    multis = Enum.reduce(stats, Ecto.Multi.new(), fn stat, multi ->
      Ecto.Multi.update(
        multi,
    ...
    multis_with_upload =
      [Ecto.Multi.update(multis, %{upload: upload.id}, upload) | multis]
